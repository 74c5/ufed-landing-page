...
export const Tracker = (elements) => {
     ...
     */
    const get_pixels_in_zone = (
        element,
        zone_top = 0,
        zone_bottom = document.documentElement.clientHeight
    ) => {
        const box = element.getBoundingClientRect();

        if (box.top < zone_top) {
            // element starts above zone
            if (box.bottom > zone_bottom) {
                // element spans entire zone
                return zone_bottom - zone_top;
            } else if (box.bottom > zone_top) {
                // element partially in zone
                return box.bottom - zone_top;
            }
        } else if (box.top < zone_bottom) {
            // starts between top and bottom
            if (box.bottom < zone_bottom) {
                // element span within zone
    ...
    const screenAndUpdate = () => {
        ...
        //TODO: use getBoundingClientRect -- for actual top position?
